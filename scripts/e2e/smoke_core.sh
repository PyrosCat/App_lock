...
if wait_lockscreen 6; then pass "lock screen shown on protected-app launch"
...
if wait_foreground "$PROTECTED_PKG" 6; then pass "PIN unlock surfaced the protected app"
...
home; sleep 1; launch_pkg "$PROTECTED_PKG"; wait_lockscreen 6 || true
...
enter_pin "$PIN"; wait_foreground "$PROTECTED_PKG" 6 && info "recovered with correct PIN" || true
home
